...
workflow SRA2FASTQ {
    ...
    emit:
    sra_files = prefetch_results_ch.map { meta, sra_file -> sra_file }
    fastq_files = fastq_results_ch.map { meta, fastq_file -> fastq_file }
    results = fastq_results_ch
    versions = SRA_PREFETCH.out.versions
        .mix(SRA_FASTERQDUMP.out.versions)
}
